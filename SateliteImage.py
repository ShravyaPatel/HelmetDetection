...
main = Tk()
main.title(" Land Cover Changes detection: An Application to Ensemble Learning")
...
data_folder = 'data'
class_names = ['Cyclone', 'Wildfire', 'Flood', 'Earthquake']
...
def imageProcessing():
    ...
    if os.path.exists(model_data_path) and os.path.exists(model_label_path):
        ...
        class_names = ["class1", "class2"]  # Add your actual class names
        data_folder = "path/to/your/data/folder"  # Update with your data folder path
...
def Model():
    ...
    text.insert(END, "Confusion Matrix of Ensemble Learning:\n")
    text.insert(END, f"{conf_matrix}\n\n")
    text.insert(END, "Classification Report of Ensemble Learning:\n")
    ...
    plt.title('Confusion Matrix of Ensemble Learning')
...
def EModel():
    ...
    if os.path.exists(model_filename):
        ...
        text.insert(END, f'MNB Classifier Model loaded from {model_filename}\n')
    else:
        ...
        text.insert(END, f'MNB Classifier Model saved to {model_filename}\n')
    ...
    # Insert results into Tkinter text widget
    text.insert(END, "MNB Classifier Confusion Matrix:\n")
    text.insert(END, f"{conf_matrix}\n\n")
    text.insert(END, "MNB Classifier Report:\n")
    text.insert(END, f"{class_report}\n")
    ...
    plt.figure(figsize=(8, 6))
    sns.heatmap(conf_matrix, annot=True, fmt="d", cmap="Blues", cbar=False)
    plt.title('Confusion Matrix of MNB Classifier')
    plt.xlabel('Predicted')
    plt.ylabel('Actual')
    plt.show()
def predict():
...
def graph():
    ...
    labels = ['Ensemble Learning', 'MNB Classifier']
...
font = ('times', 15, 'bold')
title = Label(main, text=' Land Cover Changes detection: An Application to Ensemble Learning')
...
model1Button = Button(main, text="Build & Train MNB Classifier", command=EModel)
model1Button.place(x=20,y=200)
